...
export default function AvifToJpg() {
  const handleConvert = async (file: File): Promise<{ url: string; blob: Blob }> => {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        reject(new Error("Failed to load image. AVIF may require browser support."));
      };
        ...
        <div className="space-y-4">
          <div className="p-3 rounded-[10px]" style={{ background: "var(--color-background-secondary)" }}>
            <p className="text-xs" style={{ color: "var(--color-text-secondary)" }}>
              <strong>Tip:</strong> AVIF offers excellent compression. Convert to JPG for wider browser support.
            </p>
          </div>
        </div>
      )}
