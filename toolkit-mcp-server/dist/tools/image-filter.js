...
export const ImageFilterSchema = z
    ...
    preset: z
        ...
        .optional()
        .describe("Named preset: vintage (warm, desaturated), vivid (bright, saturated), black_and_white, cinematic (dark, cool tint), soft_focus (slight blur)"),
    quality: z
        .number()
        .int()
        .min(1)
    ...
    .strict();
export async function imageFilter(params) {
    ...
    if (params.preset) {
        const preset = FILTER_PRESETS[params.preset];
        brightness = preset.brightness;
        saturation = preset.saturation;
        blurSigma = preset.blur;
        tint = preset.tint;
        if (params.preset === "black_and_white")
            grayscale = true;
    }
    const outputPath = params.output_path ?? defaultOutputPath(params.input_path, "filtered");
    ...
    if (brightness !== 1)
        appliedFilters.push(`brightness=${brightness}`);
    if (params.contrast !== 1)
        appliedFilters.push(`contrast=${params.contrast}`);
    if (saturation !== 1)
        appliedFilters.push(`saturation=${saturation}`);
    if (blurSigma > 0)
        appliedFilters.push(`blur=${blurSigma}`);
    if (grayscale)
        appliedFilters.push("grayscale");
    if (tint)
        appliedFilters.push(`tint=${tint}`);
    if (params.preset)
        appliedFilters.push(`preset=${params.preset}`);
    const summary = [
