...
export async function imageResize(params) {
    ...
    if (!targetW && !targetH) {
        return toolError("No resize dimensions specified", "Provide width, height, scale_percent, or preset.");
    }
    const outputPath = params.output_path ?? defaultOutputPath(params.input_path, "resized");
    await ensureDir(outputPath);
    const resizeOpts = {
        withoutEnlargement: true,
    };
    if (targetW)
        resizeOpts.width = targetW;
    if (targetH)
        resizeOpts.height = targetH;
