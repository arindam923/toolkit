...
export const ImageCropSchema = z
  .object({
    ...
    aspect_ratio: z
      ...
      .optional()
      .describe(
        "Aspect ratio preset. If set, width/height define the crop area size and aspect is enforced.",
      ),
    quality: z
      .number()
      .int()
...
export async function imageCrop(params: ImageCropInput) {
  ...
  const outputPath =
    params.output_path ?? defaultOutputPath(params.input_path, "cropped");
