...
export async function imageRotate(params: ImageRotateInput) {
  ...
  if (
    ...
    return toolError(
      ...
      "Provide degrees, flip_horizontal, or flip_vertical.",
    );
  }

  const outputPath =
    params.output_path ?? defaultOutputPath(params.input_path, "rotated");
  ...
  if (params.flip_horizontal || params.flip_vertical) {
    ...
    if (params.flip_vertical) pipeline = pipeline.flip();
    if (params.flip_horizontal) pipeline = pipeline.flop();
  }
  ...
  const ops: string[] = [];
  if (params.degrees !== 0) ops.push(`rotate ${params.degrees}°`);
  if (params.flip_horizontal) ops.push("flip horizontal");
  if (params.flip_vertical) ops.push("flip vertical");
